...
## Name the parameters:
par_names <- c("A", "s", "utilityP1", "utilityP2", "threshold", "t0")
...
ll <- function(x, data, sample=FALSE) {
  ...
  up1 <- pnorm(x["utilityP1"])
  up2 <- pnorm(x["utilityP2"])
  # log transform for all other parameters
  x[!(par_names %in% c("utilityP1", "utilityP2"))] <- exp(x[!(par_names %in% c("utilityP1", "utilityP2"))])
