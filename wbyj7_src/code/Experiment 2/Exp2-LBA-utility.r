...
## Name the parameters:
par_names <- c(paste(c("A", "s", "utilityP1", "utilityP2", "threshold", "t0"), "store", sep="_"),
               paste(c("A", "s", "drift_mean", "drift_sd", "threshold", "t0"), "dots", sep="_"))

## Some good starts for a chain.
start_points <- list(
  mu = log(c(2, .5, .9, .9, 2, .5,
             2, .5, 50, 5, 2, .5)),
...
ll <- function(x, data, sample=FALSE) {
  ...
  up1 <- pnorm(x["utilityP1_store"])
  up2 <- pnorm(x["utilityP2_store"])
  # log transform for all other parameters
  x[!(par_names %in% c("utilityP1_store", "utilityP2_store"))] <- exp(x[!(par_names %in% c("utilityP1_store", "utilityP2_store"))])
  ...
  t0s.store <- rep(x["t0_store"], n.store)
