...
## Name the parameters:
par_names <- c(paste(c("A", "s", "utility", "thresholdP1", "thresholdP2", "t0"), "store", sep="_"),
               paste(c("A", "s", "drift_mean", "drift_sd", "threshold", "t0"), "dots", sep="_"))

## Some good starts for a chain.
start_points <- list(
  mu = log(c(2, .5, .9, 2, 2, .5,
             2, .5, 50, 5, 2, .5)),
...
ll <- function(x, data, sample=FALSE) {
  ...
  thresholds.store <- rep(x["thresholdP1_store"], n.store)
  thresholds.store[!phase1] <- x["thresholdP2_store"]
